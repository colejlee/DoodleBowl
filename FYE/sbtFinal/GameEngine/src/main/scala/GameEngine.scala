...
class GameClass(fileName:String) {
	private val numElems:Int = Source.fromResource(fileName).getLines().toList.length
	...
	def run():Unit = {
		...
		println("Some elements may combine with themself!")
		println(s"Create all ${numElems} elements to complete the game.")
